...
<svg width="720" height="80">
  <defs>
  <circle id="node" stroke="blue" fill-opacity="0.8" r="10"></circle>
  </defs>
  <path fill="none" stroke="black" d="M40,50 L80,50"></path>
  <path fill="none" stroke="black" d="M80,50 L120,35"></path>
  <path fill="none" stroke="black" d="M80,50 L120,65"></path>
  ...
  <use  cx="40"></use>          <!-- a -->
  <use  cx="80"></use>          <!-- b -->
  <use  cx="120" cy="35"></use> <!-- c -->
  <use  cx="120" cy="65"></use> <!-- d -->
  <use  cx="160"></use>         <!-- e -->
</svg>

I would prefer to spend more of my time on higher level descriptions
of things (and have less redundancy in the source text as well).
...
Our last tool will be D3, which is a higher level tool than SVG
...
We might start with an SVG picture like this:
...
So, we can actually manipulate our original three circle demo from
above via D3 with functional code that uses a CSS selector to grab all
the circles and modify them collectively. (I have put my circles here
into an `svg` with the class `"d3_demo", so that the other circles
above are protected.)

<svg class="d3_demo" width="720" height="120">
  <circle cx="40" cy="60" r="10"></circle>
  <circle cx="80" cy="60" r="10"></circle>
  <circle cx="120" cy="60" r="10"></circle>
</svg>
...
<script>
var circle = d3.selectAll(".d3_demo circle");
...
The source text for the original SVG is unmodified;
D3 is doing the work of updating the styles and attributes
based on the selected set of circles.

However, since we're already doing Javascript, we might prefer to just
use D3 for specifying everything about our graph, (rather than, for
example, trying to put the nodes into SVG and the edges in D3).

D3 is designed to be used for (dynamic) data-driven visualization.
So its API is centered around a separation between the data you are
